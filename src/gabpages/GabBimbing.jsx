...
export default function GabBimbing() {
  ...
  const handleResultHistoryStruk = async () => {
    try {
      let temp_data = []
      const response = await fetch('https://servers-pa.vercel.app/api/resultstruk'); 
      const data = await response.json();
      for(let i = 0; i < data.length; i++){
        if(data[i].username == sessionStorage.getItem('name')){
          temp_data.push({
  ...
  const handleResultHistoryStruk3 = async () => {
    try {
      let temp_data = []
      const response = await fetch('https://servers-pa.vercel.app/api/resultstruk3'); 
      const data = await response.json();
      for(let i = 0; i < data.length; i++){
        if(data[i].username == sessionStorage.getItem('name')){
          temp_data.push({
  ...
  const handleResultHistoryStruk4 = async () => {
    try {
      let temp_data = []
      const response = await fetch('https://servers-pa.vercel.app/api/resultstruk4'); 
      const data = await response.json();
      for(let i = 0; i < data.length; i++){
        if(data[i].username == sessionStorage.getItem('name')){
          temp_data.push({
