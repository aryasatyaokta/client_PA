...
import audioBim from '../../audio/terbimbing.mp3'
import axios from 'axios'

export default function Bimbing() {
  ...
  const handleResultHistory = async () => {
    try {
      let temp_data = []
      const response = await fetch('https://servers-pa.vercel.app/api/resultbim');
  ...
  async function onRestart(){
    await axios.delete(`https://servers-pa.vercel.app/api/resultbim?name=${sessionStorage.getItem('name')}`)
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div className="col-span-2 justify-center items-center rounded-lg bg-gray-50 h-fulll dark:bg-gray-800">
              ...
              <div className='flex flex-col items-center justify-center'>
                <div className='my-16'>
                  <Link className='btn bg-[#9A3B3B] py-3 px-14 text-white rounded-md' to={'/quiz-lat-terbimbing'} onClick={startQuiz}>Start Latihan</Link>
                </div>
              </div>
            </div>
            <div>
            <div className="col-span-1 flex flex-col items-center justify-center mb-44">
              <div className="border-yellow-700 border-2 rounded-xl drop-shadow-xl w-80 flex flex-col items-center justify-center">
                ...
                <p className="text-[#9A3B3B] mt-4 text-center">Haloo, ini latihan terbimbing yang harus kamu kerjakan lhoo, semangat yaa</p>
              </div>
              <div className="mt-4">
