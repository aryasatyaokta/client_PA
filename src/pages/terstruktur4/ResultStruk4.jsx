...
import { attempts_NumberStruk4, earnPoints_NumberStruk4, flagResultStruk4 } from '../../helper/helper';
import { usePublishResultStruk4 } from '../../hooks/setResult';
import audioNilaiKurang from '../../audio/audio_nil_kurang.mp3'
import audioNilaiCukup from '../../audio/audio_nilai_cukup.mp3'
import axios from 'axios';


export default function ResultStruk4() {

    const navigate = useNavigate();
    const dispatch = useDispatch()
    const { questionStruk4: { queueStruk4, answerStruk4 }, resultStruk4: { resultsStruk4, userIdStruk3 } } = useSelector(state => state);

    const totalPoints = queueStruk4.length * 100 / 3; 
    const attempts = attempts_NumberStruk4(resultsStruk4);
    const earnPoints = earnPoints_NumberStruk4(resultsStruk4, answerStruk4, 100 / 3)
    const flag = flagResultStruk4(totalPoints, earnPoints)

    const [isAudioPlaying, setIsAudioPlaying] = useState(false);
    const [audioElement] = useState(new Audio(earnPoints >= 80 ? audioNilaiCukup : audioNilaiKurang));

    usePublishResultStruk4({ 
        resultsStruk4, 
    ...
    async function onRestart(){
      await axios.delete(`https://servers-pa.vercel.app/api/resultstruk4?name=${sessionStorage.getItem('name')}`)
            ...
            <div className="col-span-2 justify-center items-center rounded-lg bg-gray-50 h-[600px] dark:bg-gray-800">
              ...
              <div className='flex flex-col items-center my-10'>
                <div className='py-4 border-2 border-[#9A3B3B] w-1/2 h-full'>
                  ...
                  <div className='flex justify-between px-2 py-2'>
                    ...
                    <span className='font-bold'>{queueStruk4.length || 0}</span>
                  </div>
                  <div className='flex justify-between px-2 py-2'>
              ...
              <div className='flex flex-col items-center justify-center'>
                {earnPoints < 99 ? (
                  ...
                  <Link to='/lat-terbimbing' className='bg-[#9A3B3B] px-16 py-2 text-white rounded-lg'>Selanjutnya</Link>
                )}
              </div>
            </div>
            <div>
