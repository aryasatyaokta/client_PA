...
import { attempts_NumberPost2, earnPoints_NumberPost2, flagResultPost2 } from '../../helper/helper';
import { usePublishResultPost2 } from '../../hooks/setResult';
import audioNilaiKurang from '../../audio/audio_kurang_post.mp3'
import audioNilaiCukup from '../../audio/audio_belajar_selesei.mp4'
import axios from 'axios';

export default function ResultPost2() {

    const navigate = useNavigate();
    const dispatch = useDispatch()
    const { questionPost2: { queuePost2, answerPost2 }, resultPost2: { resultPost2, userIdPost2 } } = useSelector(state => state);

    const totalPoints = queuePost2.length * 20; 
    const attempts = attempts_NumberPost2(resultPost2);
    const earnPoints = earnPoints_NumberPost2(resultPost2, answerPost2, 20)
    const flag = flagResultPost2(totalPoints, earnPoints)

    const [isAudioPlaying, setIsAudioPlaying] = useState(false);
    const [audioElement] = useState(new Audio(earnPoints >= 80 ? audioNilaiCukup : audioNilaiKurang));

    usePublishResultPost2({ 
        resultPost2, 
              ...
              <div className='flex flex-col items-center my-10'>
                ...
                <div className='flex justify-between px-2'>
                  ...
                  <div className='flex justify-between px-2 py-2'>
                    ...
                    <span className='font-bold'>{queuePost2.length || 0}</span>
                  </div>
                  <div className='flex justify-between px-2 py-2'>
