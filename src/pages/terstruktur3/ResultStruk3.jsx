...
import { attempts_NumberStruk3, earnPoints_NumberStruk3, flagResultStruk3 } from '../../helper/helper';
import { usePublishResultStruk3 } from '../../hooks/setResult';
import audioNilaiKurang from '../../audio/audio_nil_kurang.mp3'
import audioNilaiCukup from '../../audio/audio_nilai_cukup.mp3'
import axios from 'axios';


export default function ResultStruk3() {

    const navigate = useNavigate();
    const dispatch = useDispatch()
    const { questionStruk3: { queueStruk3, answerStruk3 }, resultStruk3: { resultsStruk3, userIdStruk3 } } = useSelector(state => state);

    const totalPoints = queueStruk3.length * 25; 
    const attempts = attempts_NumberStruk3(resultsStruk3);
    const earnPoints = earnPoints_NumberStruk3(resultsStruk3, answerStruk3, 25)
    const flag = flagResultStruk3(totalPoints, earnPoints)

    const [isAudioPlaying, setIsAudioPlaying] = useState(false);
    const [audioElement] = useState(new Audio(earnPoints >= 80 ? audioNilaiCukup : audioNilaiKurang));

    usePublishResultStruk3({ 
        resultsStruk3, 
    ...
    async function onRestart(){
      await axios.delete(`https://servers-pa.vercel.app/api/resultstruk3?name=${sessionStorage.getItem('name')}`)
              ...
              <div className='flex flex-col items-center my-10'>
                <div className='py-4 border-2 border-[#9A3B3B] w-1/2 h-full'>
                  ...
                  <div className='flex justify-between px-2 py-2'>
                    ...
                    <span className='font-bold'>{queueStruk3.length || 0}</span>
                  </div>
                  <div className='flex justify-between px-2 py-2'>
