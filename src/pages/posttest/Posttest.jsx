...
import audioPos from '../../audio/posttest.mp3'
...
export default function Posttest() {
  ...
  const Posttest = useRef(new Audio(audioPos))
  const [isPlayingPos, setIsPlayingPos] = useState(false);
  const handleToggleAudioPos = () => {
    if (isPlayingPos) {
      Posttest.current.pause();
    } else {
      Posttest.current.play();
    }
    setIsPlayingPos(!isPlayingPos);
  };
  ...
  const handleResultHistory = async () => {
    try {
      let temp_data = []
      const response = await fetch('https://servers-pa.vercel.app/api/resultpost');
  ...
  async function onRestart(){
    await axios.delete(`https://servers-pa.vercel.app/api/resultpost?name=${sessionStorage.getItem('name')}`)
    navigate("/materii")
    navigate(0);
  }
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div className="col-span-2 justify-center items-center rounded-lg bg-gray-50 h-fulll dark:bg-gray-800">
              ...
              <div className='flex flex-col items-center justify-center'>
                <div className='start my-16'>
                  <Link className='btn bg-[#9A3B3B] py-3 px-14 text-white rounded-md' to={'/quiz-post'} onClick={startQuiz}>Start Latihan</Link>
                </div>
              </div>
            </div>
            <div className="col-span-1 flex flex-col items-center justify-center mb-44">
              <div className="border-yellow-700 border-2 rounded-xl drop-shadow-xl w-80 flex flex-col items-center justify-center">
                ...
                <p className="text-[#9A3B3B] mt-4 text-center">Haloo, ini posttest yang harus kamu kerjakan lhoo, semangat yaa</p>
              </div>
              <div className="mt-4">
                <button onClick={handleToggleAudioPos}
                  className="bg-[#9A3B3B] px-4 py-2 text-white rounded-lg"
                >
                  {isPlayingPos ? 'Pause Joko' : 'Suara Joko'}
                </button>
              </div>
            </div>
          </div>
