...
import Struktur5 from '../terstruktur5/Struktur5'
import Struktur6 from '../terstruktur6/Struktur6'

export default function Struktur2() {
  ...
  const handleResultHistory = async () => {
    try {
      let temp_data = []
      const response = await fetch('https://servers-pa.vercel.app/api/resultstruk2');
  ...
  const handleResultHistory2 = async () => {
    try {
      let temp_data = []
      const response = await fetch('https://servers-pa.vercel.app/api/resultstruk5');
      const data = await response.json();
      for(let i = 0; i < data.length; i++){
        if(data[i].username == sessionStorage.getItem('name')){
          temp_data.push({
  ...
  const handleResultHistory3 = async () => {
    try {
      let temp_data = []
      const response = await fetch('https://servers-pa.vercel.app/api/resultstruk6');
      const data = await response.json();
      for(let i = 0; i < data.length; i++){
        if(data[i].username == sessionStorage.getItem('name')){
          temp_data.push({
            ...
            <div className="col-span-2 justify-center items-center rounded-lg bg-gray-50 h-fulll dark:bg-gray-800">
              ...
              <div className='flex flex-col items-center justify-center'>
                <div className='start my-16'>
                  <Link className='btn bg-[#9A3B3B] py-3 px-14 text-white rounded-md' to={'/quiz-lat-struktur'} onClick={startQuiz}>Start Latihan</Link>
                </div>
              </div>
            </div>
